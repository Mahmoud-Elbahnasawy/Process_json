...
def process_json_object(Josn_object  , parent_table_name = 'Main_table' , nested_level=0 , tables = dict() , linking_column = ''):
    ...
    OUTPUT
        TABLES : DICTIONARY CONTAING TABLE NAME AS A KEY AND COMMA SEPERATED COLUMN NAMES  
    """
    print(f"DEBUGGING MESSAGE : ********************** THE FUNCTION {__name__}WAS CALLED.\n")
    ...
    main_object_counter = 1
    for i in json_passer(Josn_object):
        if main_object_counter == 1:
            key_counter = 1
            if nested_level == 0:
                linking_column = list(i.keys())[0]
...
def main():
    ...
    for i in json_passer(file):
        if counter <= 2:
            extracted_values  = json_values_extractor(i , 'main_table')
            ...
            cur.execute(insert_statements[0] ,main_table_values.split(",") )
            cur.execute(insert_statements[1] ,Address_table_values.split(",") )
            cur.execute(insert_statements[2] ,geo_table_Values.split(",") )
            cur.execute(insert_statements[3] ,company_table_values.split(",") )
            counter += 1
